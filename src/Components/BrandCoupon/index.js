...
const Coupon = ({ coupon }) => {
  ...
  const [counter, setCounter] = useState(
    Math.abs(
      (new Date(coupon?.endDate).getTime() - new Date().getTime()) / 1000
    )
  );
  ...
  const handleClick = (id) => {
    ...
    const boolCheck = check[0]?.clicks?.some(
      (usernew) => usernew?.userId === user?.user?._id
    );
          ...
          <BrandCoupon>
            ...
            <Div>
              <div>
                <h3>CODE</h3>
                <p>
                  {coupon?.discount}% OFF NGN {coupon?.price} min. on purchases
                </p>
              </div>
              <div>
                <Span style={{ paddingLeft: "0" }}>
                  {counter === 0 ? "0" : <> ends in {formatDays(counter)}</>}
                </Span>
                ...
                <p className={Classes.bold}>
                  {
                    allCoupons?.coupons?.filter(
                      (coup) => coup?._id === coupon._id
                    )[0]?.clicks?.length
                  }{" "}
                  clicks so far
                </p>
      ...
      >
        {user ? (
          <Grid
            ...
            alignItems="center"
            className="coupon-back module"
          >
            {click ? (
              <Grid
                className="coupon-reveal"
                onClick={() => handleClick(coupon?._id)}
              >
                {coupon?.code}
              </Grid>
            ) : (
              <h2 class="stripe-3" onClick={() => handleClick(coupon?._id)}>
                Click to Copy &amp; Reveal
              </h2>
            )}
